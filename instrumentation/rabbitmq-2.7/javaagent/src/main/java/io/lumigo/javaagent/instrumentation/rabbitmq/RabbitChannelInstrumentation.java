...
public class RabbitChannelInstrumentation implements TypeInstrumentation {
  ...
  public void transform(TypeTransformer transformer) {
    transformer.applyAdviceToMethod(
        named("basicPublish").and(isMethod()).and(takesArguments(6)),
        RabbitChannelInstrumentation.class.getName() + "$PublishAdvice");
  }

  public static class PublishAdvice {
    @Advice.OnMethodEnter(suppress = Throwable.class)
    public static void onEnter(@Advice.Argument(5) byte[] body) {
